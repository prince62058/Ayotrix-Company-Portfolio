...
export default function DigitalMarketingServiceDetail() {
  ...
  const service = apiServices ? (apiServices as any[]).find(s => s.slug === slug) : DM_SERVICES.find(d => d.slug === slug);
  ...
  if (!service) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center text-center px-4 pt-24">
        ...
        <Button asChild><Link href="/digital-marketing">Back to Digital Marketing</Link></Button>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-white pb-20">
      ...
      <section className="relative overflow-hidden pt-32 pb-24" style={{ background: service.gradient }}>
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-1/4 right-1/4 w-96 h-96 rounded-full blur-3xl opacity-20" style={{ background: service.color }} />
          <div className="absolute bottom-0 left-0 w-96 h-96 rounded-full blur-3xl opacity-10 bg-white" />
        </div>
        <div className="container mx-auto px-4 max-w-6xl relative z-10">
          <Link href="/digital-marketing" className="inline-flex items-center gap-2 text-white/70 hover:text-white text-sm font-medium mb-8 transition-colors">
            <ArrowLeft className="w-4 h-4" /> Back to Digital Marketing
          </Link>
      ...
      <section className="container mx-auto px-4 max-w-6xl py-20">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-14">
            ...
            {/* Features */}
            <ScrollReveal>
              <h2 className="text-3xl font-black mb-8" style={{ color: "#0A1628" }}>What We Do</h2>
                ...
                {service.features.map((feature, i) => (
                  <StaggerChild key={i}>
                    <div
                      ...
                      onMouseEnter={e => { (e.currentTarget as HTMLElement).style.boxShadow = `0 8px 32px ${service.color}20`; (e.currentTarget as HTMLElement).style.borderColor = `${service.color}30`; }}
            ...
            {/* Deliverables */}
            <ScrollReveal>
              <h2 className="text-3xl font-black mb-8" style={{ color: "#0A1628" }}>What You Get</h2>
              <div className="rounded-2xl p-8 border border-gray-100" style={{ background: service.bgLight }}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {service.deliverables.map((deliverable, i) => (
                    ...
                    >
                      <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0" style={{ background: "white", boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
                        <Package className="w-4 h-4" style={{ color: service.color }} />
                      </div>
                      <span className="font-medium text-sm" style={{ color: "#374151" }}>{deliverable}</span>
                    </motion.div>
                  ))}
                </div>
              </div>
